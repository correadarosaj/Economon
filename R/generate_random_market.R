# The random market draws the alpha from a truncated normal distribution

generate_random_market = function(


  condition = "",# just a label
    # SET DEFAULT VALUES
    n_trials = 1000,# how long to run the simulation
  ...
  #----------------------------------
  # SET INITIAL VALUES
  alpha_b = max(c(rnorm(1,alpha_b0,0.1*alpha_b0),0.001))
  alpha_s = alpha_s0
  market = NULL
  time_since = 1 # time since last transaction
  run_number = 1
